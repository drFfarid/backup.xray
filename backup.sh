...
if [[ "$crontabs" == "y" ]]; then
# remove cronjobs
sudo crontab -l | grep -vE '/root/fb-backup.+\.sh' | crontab -
fi


# m backup
# ساخت فایل پشتیبانی برای نرم‌افزار Marzban و ذخیره آن در فایل fb-backup.zip
if [[ "$xmh" == "m" ]]; then
...
if [ -d "/var/lib/marzban/mysql" ]; then
  ...
  source /opt/marzban/.env
    ...
    cat > "/var/lib/marzban/mysql/fb-backup.sh" <<EOL
#!/bin/bash
...
EOL
chmod +x /var/lib/marzban/mysql/fb-backup.sh

ZIP=$(cat <<EOF
docker exec marzban-mysql-1 bash -c "/var/lib/mysql/fb-backup.sh"
zip -r /root/fb-backup-m.zip /opt/marzban/* /var/lib/marzban/* /opt/marzban/.env -x /var/lib/marzban/mysql/\*
zip -r /root/fb-backup-m.zip /var/lib/marzban/mysql/db-backup/*
rm -rf /var/lib/marzban/mysql/db-backup/*
EOF
)

    else
      ZIP="zip -r /root/fb-backup-m.zip ${dir}/* /var/lib/marzban/* /opt/marzban/.env"
fi

xrayLover="marzban backup"

# x-ui backup
# ساخت فایل پشتیبانی برای نرم‌افزار X-UI و ذخیره آن در فایل fb-backup.zip
elif [[ "$xmh" == "x" ]]; then
...
ZIP="zip /root/fb-backup-x.zip ${dbDir}/x-ui.db ${configDir}/config.json"
xrayLover="x-ui backup"

# hiddify backup
# ساخت فایل پشتیبانی برای نرم‌افزار Hiddify و ذخیره آن در فایل fb-backup.zip
elif [[ "$xmh" == "h" ]]; then
...
rm -f /root/fb-backup-h.zip
zip /root/fb-backup-h.zip /opt/hiddify-config/hiddify-panel/backup/\$latest_file

EOF
)
xrayLover="hiddify backup"
else
...
cat > "/root/fb-backup-${xmh}.sh" <<EOL
rm -rf /root/fb-backup-${xmh}.zip
$ZIP
echo -e "$comment" | zip -z /root/fb-backup-${xmh}.zip
curl -F chat_id="${chatid}" -F caption=\$'${caption}' -F parse_mode="HTML" -F document=@"/root/fb-backup-${xmh}.zip" htdDocumtps://api.telegram.org/bot${tk}/senent
EOL


# Add cronjob
# افزودن کرانجاب جدید برای اجرای دوره‌ای این اسکریپت
{ crontab -l -u root; echo "${cron_time} /bin/bash /root/fb-backup-${xmh}.sh >/dev/null 2>&1"; } | crontab -u root -

# run the script
# اجرای این اسکریپت
bash "/root/fb-backup-${xmh}.sh"
